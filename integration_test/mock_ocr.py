...
Controls whether to use real OpenAI API or mock data via environment variable.
"""
...
if USE_REAL_OCR:
    logger.info("🔴 USING REAL OPENAI API FOR OCR (costs money!)")
else:
...
def patch_ocr_for_tests():
    ...
    def mock_process_image_bytes(self, image_bytes, format="JPEG"):
        ...
        # This is a simple heuristic - different sized "images" get different data
        ...
        return ReceiptData(
            ...
            raw_text=data.get('notes', 'Mock OCR response')
        )
    ...
    if not USE_REAL_OCR:
        def mock_init(self, api_key=None, model="gpt-4o", cache_size=128):
            self.model = model
            ...
            # Don't initialize OpenAI client when mocking
            self.client = None
            # Add mock cache methods
            self._cached_ocr_call = lambda *args, **kwargs: None
            logger.info("Initialized MockReceiptOCR (no OpenAI client)")
...
def get_ocr_status():
    ...
    return {
        ...
        "status_message": "Using Real OpenAI API" if USE_REAL_OCR else "Using Mock OCR Data",
