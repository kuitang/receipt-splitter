...
class ReceiptWorkflowTest(IntegrationTestBase):
    ...
    class TestData:
        """Test data specific to receipt workflow tests"""
        
        @staticmethod
        def gin_mill_receipt():
            """The Gin Mill receipt data for complete workflow testing"""
            return {
                'restaurant_name': 'The Gin Mill',
                'items': [
                    {'name': 'Nachos', 'quantity': 1, 'unit_price': '12.00', 'total_price': '12.00'},
                    {'name': 'Wings', 'quantity': 1, 'unit_price': '14.00', 'total_price': '14.00'},
                    {'name': 'Beer', 'quantity': 2, 'unit_price': '6.00', 'total_price': '12.00'}
                ],
                'subtotal': '38.00',
                'tax': '3.04',
                'tip': '7.60',
    ...
    def test_complete_workflow(self) -> TestResult:
        ...
        # Calculate claimed totals using the helper method
        claims_by_user, total_claimed = self.calculate_claims_by_user(final_data)
...
class SecurityValidationTest(IntegrationTestBase):
    ...
    class TestData:
        """Test data specific to security validation tests"""
    ...
    def test_input_validation(self) -> TestResult:
        ...
        receipt_slug = self.setup_receipt("Test Security User")
    ...
    def test_session_security(self) -> TestResult:
        ...
        receipt_slug = self.setup_receipt("User A", user_instance=user_a)
        ...
        receipt_slug2 = self.setup_receipt("User A Second", user_instance=user_a)
        ...
        receipt_slug3 = self.setup_receipt("User A Third", user_instance=user_a)
...
class ValidationTest(IntegrationTestBase):
    ...
    class TestData:
        """Test data specific to validation tests"""
        
        @staticmethod
        def test_amounts():
            """Test amounts for balance validation"""
            return {
                'valid_subtotal': '20.00',
                'valid_tax': '1.60', 
                'valid_tip': '3.40',
                'valid_total': '25.00',
                'invalid_total': '30.00'  # Unbalanced total
            }
    
    @test_wrapper
    def test_balance_validation(self):
        """Receipt Balance Validation Test"""
        # Upload receipt
        print("\n💰 Setting up test receipt")
        receipt_slug = self.setup_receipt("Test Validation User")
...
class UIValidationTest(IntegrationTestBase):
    ...
    class TestData:
        """Test data specific to UI validation tests"""
    ...
    @test_wrapper
    def test_frontend_heic_support(self):
        """UI: Frontend HEIC Support Test"""
...
class PermissionTest(IntegrationTestBase):
    ...
    class TestData:
        """Test data specific to permission tests"""
        
        @staticmethod
        def test_users():
            """User names for permission testing"""
            return {
                'uploader': 'Test User Alice',
                'claimant_bob': 'Test User Bob',
                'claimant_carol': 'Test User Carol',
                'unauthorized': 'Random User'
            }
        
        @staticmethod
        def permission_test_receipt():
            """Receipt data for permission testing"""
            return {
                'restaurant_name': 'Permission Test Restaurant',
                'items': [
                    {'name': 'Test Item 1', 'quantity': 1, 'unit_price': '15.00', 'total_price': '15.00'},
                    {'name': 'Test Item 2', 'quantity': 1, 'unit_price': '10.00', 'total_price': '10.00'}
                ],
                'subtotal': '25.00',
                'tax': '2.00',
                'tip': '3.75',
        ...
        @staticmethod
        def gin_mill_claims_receipt():
            """The Gin Mill receipt data with specific items for claims testing"""
            return {
                'restaurant_name': 'The Gin Mill',
                'subtotal': '64.00',
                'tax': '0.00',
    ...
    def test_name_based_claim_calculations(self) -> TestResult:
        ...
        receipt_slug = self.setup_receipt("Restaurant Owner", user_instance=uploader)
        print(f"   ✓ Receipt uploaded, slug: {receipt_slug}")
        
        # Update with test data
        test_data = PermissionTest.TestData.gin_mill_claims_receipt()
    ...
    def test_uploader_permissions_with_name_change(self) -> TestResult:
        ...
        uploader = self.create_new_session()
        receipt_slug = self.setup_receipt("Original Uploader", user_instance=uploader)
...
class PerformanceTest(IntegrationTestBase):
    ...
    class TestData:
        """Test data specific to performance tests"""
        
        @staticmethod
        def large_item_counts():
            """Item counts for performance testing"""
            return {
                'medium': 25,
                'large': 50,
                'extra_large': 100
    ...
    def test_large_receipt(self) -> TestResult:
        ...
        receipt_slug = self.setup_receipt("Test Performance User")
...
def get_test_classes():
    """Return a dictionary of available test class names to classes"""
    return {
        'ReceiptWorkflowTest': ReceiptWorkflowTest,
        'SecurityValidationTest': SecurityValidationTest,
        'ValidationTest': ValidationTest,
        'UIValidationTest': UIValidationTest,
        'PermissionTest': PermissionTest,
        'PerformanceTest': PerformanceTest,
    }


def run_test_classes(test_class_names=None):
    """Run specified test classes or all if none specified"""
    print("\n" + "=" * 70)
    ...
    available_classes = get_test_classes()
    
    # Determine which classes to run
    if test_class_names is None:
        classes_to_run = list(available_classes.keys())
        print(f"\n🎯 Running all test classes: {', '.join(classes_to_run)}")
    else:
        classes_to_run = []
        for class_name in test_class_names:
            if class_name in available_classes:
                classes_to_run.append(class_name)
            else:
                print(f"❌ Unknown test class: {class_name}")
                print(f"Available classes: {', '.join(available_classes.keys())}")
                return 1
        print(f"\n🎯 Running test classes: {', '.join(classes_to_run)}")
    ...
    try:
        ...
        for class_name in classes_to_run:
            ...
            test_instance = available_classes[class_name]()
            class_results = test_instance.run_tests()
            
            # Add class prefix to test names for clarity
            for test_name, result in class_results:
                full_name = f"{class_name}.{test_name}"
                results.append((full_name, result))
            
            # Add delay between test classes to avoid rate limiting
            if class_name != classes_to_run[-1]:  # Don't sleep after last class
                time.sleep(2)
...
def run_all_tests():
    """Run all integration tests (legacy function)"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run integration tests for Receipt Splitter',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  python integration_test/test_suite.py                     # Run all tests
  python integration_test/test_suite.py --list             # List available test classes  
  python integration_test/test_suite.py ReceiptWorkflowTest # Run one test class
  python integration_test/test_suite.py ValidationTest SecurityValidationTest  # Run multiple classes
        ''')
    
    parser.add_argument(
        'test_classes', 
        nargs='*', 
        help='Test class names to run (default: run all)')
    
    parser.add_argument(
        '--list', 
        action='store_true',
        help='List available test classes and exit')
    ...
    if args.list:
        ...
        for class_name in available_classes.keys():
            print(f"  - {class_name}")
        return 0
    
    # Run tests
    if args.test_classes:
        exit_code = run_test_classes(args.test_classes)
    else:
        exit_code = run_test_classes()
